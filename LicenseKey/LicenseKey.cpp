...
bool CreateMacGuidKey(std::string, std::string, std::string&, bool);
...
//#ifdef GENERATOR_BUILD
	...
	time_t startGen, endGen, startGETUSERACCOUNT, endGETUSERACCOUNT, startGETWMI, endGETWMI, startCREATELK, endCREATELK, startMACGUIDKEY, endMACGUIDKEY;  //**********************************
	double resultGen, resultGETUSERACCOUNT, resultGETWMI, resultCREATELK, resultMACGUIDKEY;  //****************************
	startGen = clock();
	...
	std::cout << "Intrusion Path : " << intrusion << "\nProfile.txt Path : " << lkeyfile << "\nInfo.txt Path : " << lkeyfileAnother << "\nid_gu.dat Path : " << macguidFile << "\nuserAsBrain : " << userAsBrain << std::endl; //************************
	...
	startGETWMI = clock();  //****************************
	GetWMIMboard(mBoard, initCOM);
	GetWMI(uuid, guid, mac, initCOM);
	endGETWMI = clock();  //****************************
	...
	if (!CreateMacGuidKey(mBoard, guid, macGuidKey, initCOM))
	{
		macGuidKey = "";
	}
	std::cout << "macGuidKey created : " << macGuidKey << "\licenseKey created : " << licenseKey << std::endl;
	endMACGUIDKEY = clock();  //****************************
...
//
//	startMACGUIDKEY = clock();  //****************************
//	std::string macGuidKey, licenseKey;
//	if (!CreateMacGuidKey(mBoard, guid, macGuidKey, initCOM) || !CreateLicenseKey(mBoard, uuid, mac, licenseKey, initCOM))
//	{
//		macGuidKey = "";
//	}
//	endMACGUIDKEY = clock();  //****************************
...
bool ValidateLicenseKey(bool initCOM)
{
	...
	std::string mBoard, uuid, mac, guid;
	startCONVERT = clock();  //****************************

	if (GetWMIMboard(mBoard, initCOM) && !filemacguid.empty())
	{
		guid = DecryptGUID(mBoard, filemacguid);
		if (ConvertGUID(guid))
		{
			endCONVERT = clock();  //****************************
	...
	std::string licenseKey = CreateLicenseKey(mBoard, uuid, mac, initCOM);
	if (!file.empty() && !fileAnother.empty() && !licenseKey.empty())
	{
		if (!PathFileExists(intrusion.c_str()) && file == licenseKey && fileAnother == licenseKey)
		{
			return true;
		}
	}
	...
	//if (!file.empty() && !fileAnother.empty() && CreateLicenseKey(mBoard, uuid, mac, licenseKey, initCOM))
	//{
	//	if (!PathFileExists(intrusion.c_str()) && file == licenseKey && fileAnother == licenseKey)
	//	{
	//		return true;
...
//
//bool CreateLicenseKey(std::string mBoard, std::string uuid, std::string mac, std::string& license, bool initCOM)
//{
...
//			{
//				license = SHA256HashString(SHA256HashString(mac2 + uuid + mBoard2) + SHA256HashString(mBoard + mac + uuid2));
//			}
//		}
...
//}
//
//bool CreateMacGuidKey(std::string mb, std::string guid, std::string& cipher, bool initCOM)
//{
...
//
//		//WriteToFile("D:\\ZipTest\\CipherText.txt", ciphertext);
//		//std::string textcipher = ReadFromFile("D:\\ZipTest\\CipherText.txt");
//		//std::cout << "textcipher : " << textcipher << std::endl;
//		//std::cout << std::endl << std::endl;
//
//		std::vector<CryptoPP::byte> byteArray(ciphertext.size());
//
//		std::cout << typeid(ciphertext).name() << std::endl;
//		for (int i = 0; i < ciphertext.size(); i++) {
//
//			std::cout << "0x" << std::hex << (0xFF & static_cast<CryptoPP::byte>(ciphertext[i])) << " ";
//			byteArray[i] = 0xFF & static_cast<CryptoPP::byte>(ciphertext[i]);
//		}
//		//CryptoPP::byte decoded[arrayLength] = {};
//
//		std::cout << "\nCipherText : " << ciphertext << std::endl;
//
//		return ciphertext;
...
std::string CreateLicenseKey(std::string mBoard, std::string uuid, std::string mac, bool initCOM)
{
	if (!(mBoard.empty() && uuid.empty() && mac.empty()))
	{
		std::string mBoard2, uuid2, mac2, license;
		if (RemoveSymbols(mBoard) && RemoveSymbols(uuid) && RemoveSymbols(mac))
		{
			std::cout << "mBoard : " << mBoard << "\tuuid : " << uuid << "\tmac : " << mac << std::endl;
			if (SplitString(mBoard, mBoard2) && SplitString(uuid, uuid2) && SplitString(mac, mac2))
			{
				std::cout << "mBoard : " << mBoard << "\tuuid : " << uuid << "\tmac : " << mac << std::endl;
				std::cout << "mBoard2 : " << mBoard2 << "\tuuid2 : " << uuid2 << "\tmac2 : " << mac2 << std::endl;
				license = SHA256HashString(SHA256HashString(mac2 + uuid + mBoard2) + SHA256HashString(mBoard + mac + uuid2));
			}
		}
		return license;
